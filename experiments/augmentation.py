...
# Define hyperparams for discriminator
...
wandb.init(
    project="Master-Thesis-Augmentation-CLR",
    config=config,
    group="experiment_1",
    job_type="eval"
)
...
for random_seed in range(10):
    print(f"iteration {random_seed+1}/10")
    ...
    for method, fun  in smote_methods.items():
        ...
        globals()[f"{method}_disc"] = RandomForestClassifier(**rf_hyperparams, random_state=random_seed)
    ...
    shuffle_disc = RandomForestClassifier(**rf_hyperparams, random_state=random_seed) 
    ...
    res_list = []
    dt = x_real_obs.clone()
    for tree in range(rf_hyperparams["n_estimators"]):
    ...
    realdata_disc = RandomForestClassifier(**rf_hyperparams, random_state=random_seed) 
